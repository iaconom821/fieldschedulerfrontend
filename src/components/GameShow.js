...
function GameShow({ deleteGame }) {
  ...
  function handleJoinGame() {
    fetch(`https://fieldschedulerbackend.herokuapp.com/api/v1/slots`, {
      method: "POST",
      headers: {
  ...
  function handleLeaveGame() {
    const slotId = slots.find(
      (slot) => parseInt(slot.player_id) === parseInt(localStorage.userId)
    ).id;

    fetch(
      `https://fieldschedulerbackend.herokuapp.com/api/v1/slots/${slotId}`,
      {
        method: "DELETE",
        headers: {
          "content-type": "application/json",
      ...
      setSlots(slots.filter((slot) => parseInt(slot.id) !== parseInt(slotId)))
    );
  }
  ...
  return (
    <>
      ...
      <StyledLabel as="p">
        Start Time:{" "}
        {`${new Date(game.start_time.split("Z")[0]).toDateString()} ${new Date(
          game.start_time.split("Z")[0]
        ).toLocaleTimeString()}`}
      </StyledLabel>
      <StyledLabel as="p">
        End Time:{" "}
        {`${new Date(game.end_time.split("Z")[0]).toDateString()} ${new Date(
          game.end_time.split("Z")[0]
        ).toLocaleTimeString()}`}
      </StyledLabel>
      <StyledDiv>
        <img
          style={{ margin: "16px" }}
          src={game.field.img_url}
          alt={game.field.id}
        />
      ...
      <StyledBottomDiv>
        ...
        {parseInt(localStorage.userId) === parseInt(game.player_id) ? (
          <StyledButton onClick={handleDeleteGame}>Delete Game</StyledButton>
        ) : null}
        {parseInt(localStorage.userId) === parseInt(game.player_id) ? (
          <StyledLinkTo to={`${match.url}/${game.id}/edit`}>
            Edit Game
          </StyledLinkTo>
        ) : null}
      </StyledBottomDiv>
